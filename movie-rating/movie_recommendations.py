...
class MovieDB(object):
    ...
    def get_movie_name( self, movie_id):
        if movie_id not in self.names:
            self.names[movie_id] = movie_data_helper.get_movie_name(movie_id)
...
def compute_posterior(prior, likelihood, y):
    ...
    N = np.size(y)
    log_answer = np.copy(log_prior)
    log_p_y_given_x = np.empty( N )
    for x in range(M):
        pdist_y_given_x  = likelihood[:, x]
        log_p_y_given_x = [ np.log(pdist_y_given_x[e])  for e in y ]
        log_answer[ x ] = log_answer[ x ] + np.sum( log_p_y_given_x )
    
    log_answer  = np.subtract( log_answer, scipy.misc.logsumexp( log_answer) )
...
def compute_movie_rating_likelihood(M):
    ...
    for x in range(M):
        ...
        s = np.sum(likelihood[:,x])       
        likelihood[:,x] = np.divide(likelihood[:,x], s )    
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    movie_id_list = movieDB.get_movie_id_list()
    ...
    #MAP_ratings = [ np.argmax( movie_posterior) for movie_posterior in posteriors ]
    #print (MAP_ratings)
    i = 0
    for movie_posterior in posteriors :
        MAP_ratings[ i ] = np.argmax( movie_posterior)
        i = i+1
...
def compute_true_movie_rating_posterior_entropies(num_observations):
    ...
    N,M = posteriors.shape
    posterior_entropies = []
    
    for p_dist in posteriors:
        e = compute_entropy( p_dist )
        posterior_entropies.append( e )
...
def generatePlot():
    ...
    N =  range( 1,201)
    for n in N:
       posterior_entropies =  compute_true_movie_rating_posterior_entropies( n )
       e = sum(posterior_entropies)/len( posterior_entropies)
       num_entropy.append( e )
       
    plt.figure(1)
    plt.title("Average of Entropies")
    plt.xlabel("Number of Samples")
    plt.ylabel("E (bits)", rotation='horizontal', position=(0, 1.01))
    plt.plot(N, num_entropy )
    plt.show()
...
#    generatePlot()
    ...
    top_rated_movies = np.array(Map_ratings).argsort() [::-1][:10]
    print(top_rated_movies)
    movie_names =  np.empty(10)
    movie_names = [ movie_data_helper.get_movie_name( movie_id) for movie_id in top_rated_movies ]
    print( movie_names )
